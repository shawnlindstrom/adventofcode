...
function santaInteger($input, $depth, $zeroes)
{
    for ($i = 0; $i < $depth; $i++) {
        $found = strpos(md5($input . $i), $zeroes);
        if ($found === 0) return $i;
    }

    return "Search depth inadequate. Try a bigger number.";
}

echo 'Part One: ' . santaInteger($input, 1000000, "00000") . PHP_EOL;
echo 'Part Two: ' . santaInteger($input, 10000000, "000000") . PHP_EOL;