...
$input = new SplFileObject("input/day2input.txt");
$sqft = 0;
$bowLength = 0;
$ribbonLength = 0;
$ribbonNeeded = 0;
while (!$input->eof()) {
    $line = $input->fgets();
    $line = array_map('intval', explode('x', $line));

    $a = $line[0] * $line[1]; // w*l
    $b = $line[1] * $line[2]; // l*h
    $c = $line[2] * $line[0]; // h*w
    $slack = min($a, $b, $c); // find smallest side
    $sqft += $a * 2 + $b * 2 + $c * 2 + $slack;

    // part two
    $bowLength += array_product($line); // w*l*h
    $key = array_search(max($line), $line); // find the key for the max value
    unset($line[$key]); // forget the key for the max value
    $ribbonLength += array_sum($line) * 2; // shortest two sides * 2
}
$ribbonNeeded = $ribbonLength + $bowLength;
echo 'Wrapping Paper Needed: ' . $sqft . ' sqft' . PHP_EOL . 'Ribbon Needed: ' . $ribbonNeeded . ' feet';
